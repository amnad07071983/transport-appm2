...
# แก้ไข: เพิ่ม TTL เป็น 300 วินาที (5 นาที) เพื่อลดภาระ API อย่างหนัก
...
def get_data_cached():
    try:
        ...
        return pd.DataFrame(inv), pd.DataFrame(items), ws_inv, ws_item
    except Exception as e:
        # หากติด Error 429 ให้คืนค่าแจ้งเตือน
        st.error(f"⚠️ API Busy (Quota Limit): ระบบจะใช้ข้อมูลในหน่วยความจำชั่วคราวแทน")
        return pd.DataFrame(), pd.DataFrame(), None, None

inv_df, item_df, ws_inv, ws_item = get_data_cached()

# กรณีที่ดึงข้อมูลไม่ได้เลยในครั้งแรก
if ws_inv is None:
    st.warning("ขณะนี้ Google API จำกัดการเข้าถึงชั่วคราว กรุณารอ 30-60 วินาทีแล้วลองกดปุ่ม Refresh ด้านล่าง")
    if st.button("🔄 ลองอัปเดตข้อมูลอีกครั้ง"):
        st.cache_data.clear()
        st.rerun()
    st.stop()
...
def generate_pdf_file(inv_no, items, data_dict=None):
    ...
    w, h = A4
    page_labels = ["แผ่นที่ 1 - ต้นฉบับ - ผู้รับน้ำมัน (ปลายทาง)", "แผ่นที่ 2 - สำเนา - พนักงานขับรถ / ผู้ขนส่ง", "แผ่นที่ 3 - สำเนา - ฝ่ายบัญชี / ส่วนกลางผู้ส่ง", "แผ่นที่ 4 - สำเนา - คลังน้ำมัน (ต้นทาง)"]
    ...
    for idx, label in enumerate(page_labels):
        ...
        c.restoreState()
        c.setFont(FONT_NAME, 10); c.drawString(1.5*cm, h-0.8*cm, label)

        try:
            if os.path.exists('p1.png'):
                c.saveState(); c.setFillAlpha(0.2); img_size = 10*cm
                c.drawImage('p1.png', (w-img_size)/2, ((h-img_size)/2)-(1.5*inch), width=img_size, height=img_size, mask='auto')
                c.restoreState()
        except: pass

        c.setFont(FONT_NAME, 14); c.drawString(1.5*cm, h-1.5*cm, "1.ผู้จำหน่าย")
        ...
        c.setFont(FONT_NAME, 18); c.drawRightString(19.5*cm, h-1.7*cm, f"{get_val('ผู้จำหน่าย-ชื่อเอกสาร', 'ใบกำกับขนส่งน้ำมัน')}")
        c.setFont(FONT_NAME, 12); c.drawRightString(19.5*cm, h-2.2*cm, f"{get_val('ผู้จำหน่าย-อธิบายเพิ่ม')}")
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.2*cm, h-4.7*cm, "  2.คลังรับน้ำมัน (ต้นทาง)")
        c.setFont(FONT_NAME, 11)
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.5*cm, h-7.0*cm, "3.ตั๋วขนย้ายน้ำมัน")
        c.setFont(FONT_NAME, 11)
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.5*cm, h-9.8*cm, "4.ผู้รับน้ำมัน (ปลายทาง)")
        c.setFont(FONT_NAME, 11)
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.2*cm, h-12.4*cm, "  5.ข้อมูลการขนส่ง")
        c.setFont(FONT_NAME, 11)
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.2*cm, h-18.0*cm, "  6.รายละเอียดน้ำมันเชื้อเพลิง")
        header = [["ลำดับ", "ช่องถัง", "ซีล", "รายการน้ำมัน", "หน่วย", "จำนวน"]]
        data_rows = []; total_qty = 0.0
        for i, it in enumerate(items):
            try:
                qv = float(str(it.get('qty', '0')).replace(',', ''))
                total_qty += qv; f_qty = "{:,.0f}".format(qv)
        ...
        c.setFont(FONT_NAME, 14); c.drawString(1.2*cm, h-23.0*cm, "  7.การยืนยันและรับสินค้า")
        c.setFont(FONT_NAME, 11); c.drawString(1.5*cm, h-23.8*cm, "ข้าพเจ้าได้รับสินค้าตามรายการข้างต้นในสภาพเรียบร้อย ถูกต้องตามจำนวนและหมายเลขซีลที่ระบุไว้")
        ...
        c.setFont(FONT_NAME, 11)
        c.drawCentredString(4.5*cm, sig_y-1.2*cm, "ผู้ออกใบกำกับขนส่งน้ำมัน"); c.drawCentredString(4.5*cm, sig_y-1.7*cm, "วันที่ : ..................................")
        c.drawCentredString(10.5*cm, sig_y-1.2*cm, "ผู้ดำเนินการขนส่งน้ำมัน"); c.drawCentredString(10.5*cm, sig_y-1.7*cm, "วันที่ : ..................................")
        c.drawCentredString(16.5*cm, sig_y-1.2*cm, "ผู้รับสินค้า"); c.drawCentredString(16.5*cm, sig_y-1.7*cm, "วันที่ : ..................................")
        c.rect(1*cm, 1*cm, 19*cm, h-2*cm); c.showPage()
    c.save(); buf.seek(0)
    return buf
...
with col_link:
    st.write("") 
    st.link_button("📂 เปิด Sheets", f"https://docs.google.com/spreadsheets/d/{SHEET_ID}", use_container_width=True)
    ...
    if not inv_df.empty:
        ...
        if selected:
            ...
            if col_b.button("🔄 โหลดมาสร้างซ้ำ"):
                ...
                for f in transport_fields: st.session_state[f"in_{f}"] = str(row_data.get(f, ""))
                st.session_state.invoice_items = [{"product": i.get('product',''), "unit": i.get('unit',''), "qty": i.get('qty',''), "tank": str(i.get('tank','')), "seal": str(i.get('seal',''))} for i in it_rows]
                st.session_state.pdf_buffer = None; st.rerun()
            quick_pdf = generate_pdf_file(sel_no, it_rows, data_dict=row_data)
            col_c.download_button("📥 ดาวน์โหลด PDF", data=quick_pdf, file_name=f"Invoice_{sel_no}.pdf", mime="application/pdf")

tabs = st.tabs(["📦 ข้อมูลปลายทาง", "🚛 ผู้ขนส่ง", "⛽ รายการสินค้า", "🏢 ผู้จัดจำหน่าย"])
with tabs[0]:
...
with tabs[2]:
    ...
    if st.session_state.invoice_items:
        ...
        if st.button("🗑️ ล้างทั้งหมด"): st.session_state.invoice_items = []; st.rerun()
...
if st.button("💾 บันทึกและสร้าง PDF", type="primary", use_container_width=True):
    ...
    try:
        ...
        time.sleep(1) # ให้ API ได้พัก
        st.cache_data.clear(); st.rerun()
    except Exception as e:
        st.error(f"❌ บันทึกไม่สำเร็จ (API Limit): กรุณารอ 1 นาทีแล้วลองใหม่")
